...
void HeartbeatGenerator::StartHeartbeat(const ServerTime& startTime)
{
    ...
    m_HeartbeatCount = 0;
    GameClient::Instance().GetScheduledTaskWorkerRoutine().AddTaskAfter(m_AllowanceTime, new TaskCheckHeartbeatTimeOver(m_GameClientObject, m_HeartbeatCount));
...
void HeartbeatGenerator::KeepHeartbeat(const INT64 heartbeatCount, const ServerTime& heartbeatTime)
{
    ...
    GameClient::Instance().GetScheduledTaskWorkerRoutine().AddTaskAfter(m_Period, new TaskSendHeartbeat(m_GameClientObject,m_HeartbeatCount));
}
