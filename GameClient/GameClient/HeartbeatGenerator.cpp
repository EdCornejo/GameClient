...
void HeartbeatGenerator::StartHeartbeat(const ServerTime& startTime)
{
    ...
    m_GameClientObject->SendCSRequestHeartbeat(m_HeartbeatCount);
}
...
BOOL HeartbeatGenerator::CheckHeartbeatTimeOver(const INT64 heartbeatCount, const ServerTime& checkTime)
{
    ...
    {
        ASSERT_DEBUG(false);
        LOG_STDOUT( std::cout << "CheckHeartbeatTimeOver : heartbeatCount :" << m_HeartbeatCount << std::endl; );
        return true;
    }
    if( checkTime-m_LastHeartbeatTime > m_AllowanceTime )
    {
        ASSERT_DEBUG(false);
        LOG_STDOUT( std::cout << "CheckHeaertbeatTimeOver : " << (checkTime-m_LastHeartbeatTime).count() << std::endl; );
        return true;
    }
...
void HeartbeatGenerator::KeepHeartbeat(const INT64 heartbeatCount, const ServerTime& heartbeatTime)
{
    ...
    {
        LOG_STDOUT( std::cout << "KeepHeartbeat Time Error :" << (heartbeatTime-m_LastHeartbeatTime).count() << "LastHeartbeat : " << m_LastHeartbeatTime.count() <<  std::endl; );
        return;
    }
